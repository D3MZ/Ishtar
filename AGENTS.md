...
- `src/Ishtar.jl`: primary module; add new submodules here and export public API.
- `test/runtests.jl`: entry point for tests; group cases with `@testset`.
- `docs/`: Documenter.jl setup (`make.jl`, `docs/src/index.md`) for user docs.
- `benchmark/` and `data/`: keep reproducible scripts and sample datasets; avoid committing large/raw secrets.

## Build, Test, and Development Commands
- Install deps once: `julia --project -e 'using Pkg; Pkg.instantiate()'`.
- Run tests: `julia --project -e 'using Pkg; Pkg.test()'` (what CI runs).
- Build docs locally: `julia --project=docs -e 'using Pkg; Pkg.instantiate(); include("docs/make.jl")'`.
- Format/check docs examples: run doctests via `julia --project=docs --color=yes docs/make.jl` (also executed in CI).

## Coding Style & Naming
Follow these principles:
- Don't write docs on new code.
- Ask before adding new packages, most of the time native code is prefered. Check what packages are added already to use them in the code.
...
- For module being developed, don't preface the package name before the function call. Export the function and reference it. 
...
- Prefer fewer arguments in functions. Ideally, aim for no more than two or
  three.
- Do not write comments. The code should self document.
...
- Don't use this package unless it's already present in the codebase.


## Testing Guidelines
- Organize tests into `@testset` blocks per feature; mirror code layout in `test/` subfiles if it grows.
- For new behaviors, add regression tests rather than changing existing expectations.
- Aim for coverage on public API; seed-dependent code should set RNG seeds for repeatability.

## Commit & Pull Request Guidelines
- Commits: short, present-tense summaries (e.g., `Add walk-forward engine`, matching existing history).
- PRs: include a brief goal, bullet list of changes, and how to reproduce test/doc runs.
- Link related issues; add screenshots or text output when behavior is user-visible.
- Ensure CI passes (tests + docs). If skipping checks, explain why.

## Automation & CI Notes
- GitHub Actions runs on Julia 1.6, 1.12, and `pre`; failures on any matrix item block merge.
- Docs deploy via Documenter; requires `DOCUMENTER_KEY`/`GITHUB_TOKEN` only in CI—never commit secrets locally.
